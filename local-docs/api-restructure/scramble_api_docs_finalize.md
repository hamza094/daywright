...
**Decision: Authorization-based access control with dedicated resource**

- Created `PublicUserProfileResource` for the `/users/{user}` endpoint
- **PublicUserProfileResource data:** `id`, `uuid`, `name`, `username`, `avatar`, `timezone`, `email`, `info` (mobile, address, company, position, bio), `created_at`, `updated_at`
- **UserProfileResource (unchanged):** Used elsewhere in the application, retains original behavior including `verified` field
- **Authorization:** Added `UsersPolicy::view()` method to check for project or team membership
- **Controller:** Updated `UserController::show()` to use `PublicUserProfileResource` and authorize with `view` policy
- **Collaboration check:** Users must share project membership OR team membership to access the endpoint

This approach:

- Preserves legitimate collaboration use cases (team members can see each other's contact info)
- Prevents unauthorized PII access (random third-party clients with team:read cannot harvest PII)
- Maintains separation of concerns (no database queries in resources)
- Keeps existing `UserProfileResource` unchanged for other use cases
- Provides clear authorization boundary in policy layer

Alternative approaches considered but not selected:

- Tiered data access in resource: Would violate backend guidelines (no queries in resources)
- Restricted public profile: Would break legitimate team collaboration use cases

### 2. Production documentation access

**Decision: Public runtime documentation (Option C)**

- Allow unauthenticated access to the `/docs` endpoint in production
- Removed `RestrictedDocsAccess::class` middleware from Scramble configuration
- Third-party developers can browse interactive documentation without authentication
- This requires ensuring the generated document contains no internal routes or sensitive examples (verified in Phase 6)

Alternative models considered but not selected:

- Static public specification: Would require separate deployment pipeline
- Gated runtime documentation: Would require defining viewApiDocs gate
...
- Production documentation access policy implemented: Public runtime documentation (Option C) - removed `RestrictedDocsAccess::class` middleware to allow unauthenticated `/docs` access.
