...
Current readiness assessment: **ACTION REQUIRED**.
...
### Issue C: Stable Codes Are Not Emitted

The registry defines stable idempotency codes, but the vendor middleware throws generic HTTP exceptions. The application handler maps those to `bad_request`, `conflict`, or `validation_error`, not the documented idempotency codes.

### Resolution

1. Inspect the installed idempotency package's extension points and exception behavior.
2. Add an application-owned idempotency boundary. Prefer one of these approaches, in order:
   - Configure the package to throw application exceptions, if officially supported.
   - Replace its route alias with an application middleware that delegates to the package and translates typed vendor exceptions.
   - Implement the small public API idempotency boundary locally if the package exposes no stable typed exceptions.
3. Do not map failures by parsing vendor exception messages.
4. Introduce typed Daywright exceptions for:
   - Missing key: `400 idempotency_key_required`
   - Request in progress: `409 idempotency_in_progress`
   - Key reused with different input: `422 idempotency_key_reused`
5. Ensure the runtime formatter includes `Retry-After` for the in-progress response when a retry duration is known.
6. Make the middleware-aware Scramble extension add the request contract to every idempotent public route:
   - Required `Idempotency-Key` header
   - Type `string`
   - Business description and realistic example
7. Add the optional `Idempotency-Replayed` response header to every successful response status the route can return. For conversation creation, this is `201`, not `200`.
8. Add the documented `400`, `409`, and `422` responses with exact stable-code examples.
9. Add `Retry-After` to the documented `409` response when runtime emits it.
10. Remove redundant controller attributes after the middleware extension is proven to generate the complete contract.

### Required Tests

Runtime tests must trigger all three failures and assert status, exact code, envelope, metadata, and headers.

Generated-document tests must assert:

- `Idempotency-Key` is in `parameters` with `in: header` and `required: true`.
- The parameter schema is `string` and contains the expected example.
- `Idempotency-Replayed` exists under the `201` response headers.
- `400`, `409`, and `422` contain the canonical envelope schema.
- Each response contains the correct idempotency example code.
- `409` documents `Retry-After` when supported at runtime.

The current test computes `$hasIdempotencyKey` without asserting it. Replace that with a strict assertion.

### Acceptance Criteria

- Runtime and OpenAPI expose the same three stable idempotency codes.
- Every idempotent public operation receives the contract automatically.
...
- [ ] The repository contains no unexplained deleted application files.
- [ ] All backend tests pass, including task creation.
- [ ] All protected operations declare bearer security.
- [ ] All public operations remain unauthenticated.
- [ ] Idempotency request and response headers are generated correctly.
- [ ] Runtime emits every documented stable idempotency code.
...
Until every checkbox is complete, the correct readiness verdict remains **ACTION REQUIRED**.
