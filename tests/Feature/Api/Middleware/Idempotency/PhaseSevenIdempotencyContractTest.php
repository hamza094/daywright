...
final class PhaseSevenIdempotencyContractTest extends TestCase
{
    ...
    public function subscription_creation_replays_without_calling_the_provider_twice(): void
    {
        ...
        $firstResponse = $this->withHeaders($headers)->postJson($this->apiV1Route('subscriptions.store'), $payload);
        $secondResponse = $this->withHeaders($headers)->postJson($this->apiV1Route('subscriptions.store'), $payload);
    ...
    public function zoom_webhook_update_replays_without_queuing_the_job_twice(): void
    {
        ...
        $firstResponse = $this->postJson(route('api.v1.webhooks.meetings.update'), $payload, $headers);
        $secondResponse = $this->postJson(route('api.v1.webhooks.meetings.update'), $payload, $headers);

        $object = $payload['payload']['object'];
        $meetingId = $object['id'];
        $updateData = collect($object)->except(['id', 'uuid'])->toArray();

        $firstResponse->assertOk()->assertExactJson(['message' => 'Webhook accepted.']);
        $secondResponse->assertOk()->assertExactJson(['message' => 'Webhook accepted.']);
